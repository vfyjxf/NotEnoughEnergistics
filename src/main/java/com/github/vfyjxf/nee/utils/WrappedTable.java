...
public class WrappedTable<R, C, V> extends Table<R, C, V> {
    ...
    public WrappedTable(Table<R, C, V> other, IRecipeTransferHandlerHelper helper) {
    ...
    public V onValueSet(R row, V value) {
        ...
        if (row == PatternTermContainer.class && "appeng.integration.modules.jei.PatternRecipeTransferHandler".equals(value.getClass().getCanonicalName())) {
        ...
        if (row == CraftingTermContainer.class && "appeng.integration.modules.jei.CraftingRecipeTransferHandler".equals(value.getClass().getCanonicalName())) {
            value = (V) new CraftingHelperTransferHandler(this.helper);
            NotEnoughEnergistics.logger.info("AE2 CraftingRecipeTransferHandler Replaced Successfully (Overwrite Denied)");
        }
